...
func (devices *DeviceSet) ensureImage(name string, size int64) (string, error) {
	...
	if _, err := os.Stat(filename); err != nil {
		...
		shared.Debugf("Creating loopback file %s for device-manage use", filename)
...
func (devices *DeviceSet) deviceFileWalkFunction(path string, finfo os.FileInfo) error {
	...
	if strings.HasSuffix(finfo.Name(), ".migrated") {
		shared.Debugf("Skipping file %s", path)
		return nil
	}
	...
	shared.Debugf("Loading data for file %s", path)
	...
	if dinfo.DeviceId > MaxDeviceId {
		shared.Debugf("Ignoring Invalid DeviceId=%d", dinfo.DeviceId)
		return nil
	}
	...
	shared.Debugf("Added deviceId=%d to DeviceIdMap", dinfo.DeviceId)
	return nil
}

func (devices *DeviceSet) constructDeviceIdMap() error {
	shared.Debugf("[deviceset] constructDeviceIdMap()")
	defer shared.Debugf("[deviceset] constructDeviceIdMap() END")

	var scan = func(path string, info os.FileInfo, err error) error {
		if err != nil {
			shared.Debugf("Can't walk the file %s", path)
			return nil
		}
...
func (devices *DeviceSet) unregisterDevice(id int, hash string) error {
	shared.Debugf("unregisterDevice(%v, %v)", id, hash)
	info := &DevInfo{
		Hash:     hash,
		DeviceId: id,
	}
	...
	if err := devices.removeMetadata(info); err != nil {
		shared.Debugf("Error removing metadata: %s", err)
		return err
	}
...
func (devices *DeviceSet) registerDevice(id int, hash string, size uint64, transactionId uint64) (*DevInfo, error) {
	shared.Debugf("registerDevice(%v, %v)", id, hash)
	info := &DevInfo{
		Hash:          hash,
		DeviceId:      id,
		Size:          size,
...
func (devices *DeviceSet) activateDeviceIfNeeded(info *DevInfo) error {
	shared.Debugf("activateDeviceIfNeeded(%v)", info.Hash)
	defer shared.Debugf("activateDeviceIfNeeded(%v) END", info.Hash)
...
func (devices *DeviceSet) createRegisterDevice(hash string) (*DevInfo, error) {
	...
	if err := devices.openTransaction(hash, deviceId); err != nil {
		shared.Debugf("Error opening transaction hash = %s deviceId = %d", hash, deviceId)
	...
	for {
		if err := devicemapper.CreateDevice(devices.getPoolDevName(), deviceId); err != nil {
			if devicemapper.DeviceIdExists(err) {
				...
				shared.Debugf("Device Id %d exists in pool but it is supposed to be unused", deviceId)
				deviceId, err = devices.getNextFreeDeviceId()
				if err != nil {
					return nil, err
				}
			...
			shared.Debugf("Error creating device: %s", err)
			devices.markDeviceIdFree(deviceId)
			return nil, err
		}
		break
	...
	shared.Debugf("Registering device (id %v) with FS size %v", deviceId, devices.baseFsSize)
...
func (devices *DeviceSet) createRegisterSnapDevice(hash string, baseInfo *DevInfo) error {
	...
	for {
		if err := devicemapper.CreateSnapDevice(devices.getPoolDevName(), deviceId, baseInfo.Name(), baseInfo.DeviceId); err != nil {
			...
			shared.Debugf("Error creating snap device: %s", err)
			devices.markDeviceIdFree(deviceId)
			return err
		}
		break
	...
	if _, err := devices.registerDevice(deviceId, hash, baseInfo.Size, devices.OpenTransactionId); err != nil {
		devicemapper.DeleteDevice(devices.getPoolDevName(), deviceId)
		devices.markDeviceIdFree(deviceId)
		shared.Debugf("Error registering device: %s", err)
		return err
	}
...
func getDeviceUUID(device string) (string, error) {
	...
	if err != nil {
		shared.Debugf("Failed to find uuid for device %s:%v", device, err)
		return "", err
	}

	uuid := strings.TrimSuffix(string(out), "\n")
	uuid = strings.TrimSpace(uuid)
	shared.Debugf("UUID for device: %s is:%s", device, uuid)
	return uuid, nil
}
...
func (devices *DeviceSet) setupBaseImage() error {
	...
	if oldInfo != nil && !oldInfo.Initialized {
		shared.Debugf("Removing uninitialized base image")
	...
	shared.Debugf("Initializing base device-mapper thin volume")
	...
	shared.Debugf("Creating filesystem on base device-mapper thin volume")
...
func (devices *DeviceSet) DMLog(level int, file string, line int, dmError int, message string) {
	...
	if level <= devicemapper.LogLevelErr {
		shared.Debugf("libdevmapper(%d): %s:%d (%d) %s", level, file, line, dmError, message)
...
func (devices *DeviceSet) rollbackTransaction() error {
	shared.Debugf("Rolling back open transaction: TransactionId=%d hash=%s device_id=%d", devices.OpenTransactionId, devices.DeviceIdHash, devices.DeviceId)
	...
	if err := devicemapper.DeleteDevice(devices.getPoolDevName(), devices.DeviceId); err != nil {
		shared.Debugf("Unable to delete device: %s", err)
	}

	dinfo := &DevInfo{Hash: devices.DeviceIdHash}
	if err := devices.removeMetadata(dinfo); err != nil {
		shared.Debugf("Unable to remove metadata: %s", err)
	} else {
		devices.markDeviceIdFree(devices.DeviceId)
	}

	if err := devices.removeTransactionMetaData(); err != nil {
		shared.Debugf("Unable to remove transaction meta file %s: %s", devices.transactionMetaFile(), err)
	}
...
func (devices *DeviceSet) processPendingTransaction() error {
	...
	if devices.OpenTransactionId < devices.TransactionId {
		shared.Debugf("Open Transaction id %d is less than pool transaction id %d", devices.OpenTransactionId, devices.TransactionId)
		return nil
	}
...
func (devices *DeviceSet) closeTransaction() error {
	if err := devices.updatePoolTransactionId(); err != nil {
		shared.Debugf("Failed to close Transaction")
		return err
	}
	return nil
}
...
func determineDriverCapabilities(version string) error {
	...
	shared.Debugf("devicemapper: driver version is %s", version)
...
func getDeviceMajorMinor(file *os.File) (uint64, uint64, error) {
	...
	shared.Debugf("[devmapper]: Major:Minor for device: %s is:%v:%v", file.Name(), majorNum, minorNum)
...
func getLoopFileDeviceMajMin(filename string) (string, uint64, uint64, error) {
	...
	if err != nil {
		shared.Debugf("[devmapper]: Failed to open file %s", filename)
		return "", 0, 0, err
	}
...
func (devices *DeviceSet) getThinPoolDataMetaMajMin() (uint64, uint64, uint64, uint64, error) {
	...
	shared.Debugf("[devmapper]: poolDataMajMin=%s poolMetaMajMin=%s\n", poolDataMajMin, poolMetadataMajMin)
...
func (devices *DeviceSet) initDevmapper(doInit bool) error {
	...
	if EnableDeferredRemoval && DriverDeferredRemovalSupport && devicemapper.LibraryDeferredRemovalSupport == true {
		shared.Debugf("devmapper: Deferred removal support enabled.")
		devices.deferredRemove = true
	}

	// https://github.com/docker/docker/issues/4036
	if supported := devicemapper.UdevSetSyncSupport(true); !supported {
		shared.Debugf("Udev sync is not supported. This will lead to unexpected behavior, data loss and errors. For more information, see https://docs.docker.com/reference/commandline/cli/#daemon-storage-driver-option")
	}
	...
	// lxd-maj,min[-inode] stands for:
	//	- Managed by lxd
	//	- The target of this device is at major <maj> and minor <min>
	//	- If <inode> is defined, use that file inside the device as a loopback image. Otherwise use the device itself.
	devices.devicePrefix = fmt.Sprintf("lxd-%d:%d-%d", major(sysSt.Dev), minor(sysSt.Dev), sysSt.Ino)
	shared.Debugf("Generated prefix: %s", devices.devicePrefix)

	// Check for the existence of the thin-pool device
	shared.Debugf("Checking for existence of the pool '%s'", devices.getPoolName())
	info, err := devicemapper.GetInfo(devices.getPoolName())
	if info == nil {
		shared.Debugf("Error device devicemapper.GetInfo: %s", err)
		return err
	}
	...
	shared.Debugf("info.Exists is '%v' and devices.thinPoolDevice is '%s' (should be 0, '')", info.Exists, devices.thinPoolDevice)
	if info.Exists == 0 && devices.thinPoolDevice == "" {
		shared.Debugf("Pool doesn't exist. Creating it.")
		...
		if devices.dataDevice == "" {
			...
			if err != nil {
				shared.Debugf("Error device ensureImage (data): %s", err)
				return err
			}
		...
		if devices.metadataDevice == "" {
			...
			if err != nil {
				shared.Debugf("Error device ensureImage (metadata): %s", err)
				return err
			}
	...
	if info.Exists != 0 && devices.thinPoolDevice == "" {
		if err := devices.loadThinPoolLoopBackInfo(); err != nil {
			shared.Debugf("Failed to load thin pool loopback device information:%v", err)
			return err
		}
	}
	...
	if doInit {
		if err := devices.setupBaseImage(); err != nil {
			shared.Debugf("Error device setupBaseImage: %s", err)
			return err
		}
	}
...
func (devices *DeviceSet) AddDevice(hash, baseHash string) error {
	shared.Debugf("[deviceset] AddDevice(hash=%s basehash=%s)", hash, baseHash)
	defer shared.Debugf("[deviceset] AddDevice(hash=%s basehash=%s) END", hash, baseHash)
...
func (devices *DeviceSet) deleteDevice(info *DevInfo) error {
	if devices.doBlkDiscard {
		...
		if err := devices.activateDeviceIfNeeded(info); err == nil {
			if err := devicemapper.BlockDeviceDiscard(info.DevName()); err != nil {
				shared.Debugf("Error discarding block on device: %s (ignoring)", err)
			}
		}
	}
	...
	if devinfo != nil && devinfo.Exists != 0 {
		if err := devices.removeDevice(info.Name()); err != nil {
			shared.Debugf("Error removing device: %s", err)
			return err
		}
	}

	if err := devices.openTransaction(info.Hash, info.DeviceId); err != nil {
		shared.Debugf("Error opening transaction hash = %s deviceId = %d", "", info.DeviceId)
		return err
	}

	if err := devicemapper.DeleteDevice(devices.getPoolDevName(), info.DeviceId); err != nil {
		shared.Debugf("Error deleting device: %s", err)
		return err
	}
...
func (devices *DeviceSet) deactivatePool() error {
	shared.Debugf("[devmapper] deactivatePool()")
	defer shared.Debugf("[devmapper] deactivatePool END")
	...
	if d, err := devicemapper.GetDeps(devname); err == nil {
		...
		shared.Debugf("[devmapper] devicemapper.GetDeps() %s: %#v", devname, d)
	}
	if devinfo.Exists != 0 {
...
func (devices *DeviceSet) deactivateDevice(info *DevInfo) error {
	shared.Debugf("[devmapper] deactivateDevice(%s)", info.Hash)
	defer shared.Debugf("[devmapper] deactivateDevice END(%s)", info.Hash)
...
func (devices *DeviceSet) removeDevice(devname string) error {
	...
	shared.Debugf("[devmapper] removeDevice START(%s)", devname)
	defer shared.Debugf("[devmapper] removeDevice END(%s)", devname)
...
func (devices *DeviceSet) cancelDeferredRemoval(info *DevInfo) error {
	...
	shared.Debugf("[devmapper] cancelDeferredRemoval START(%s)", info.Name())
	defer shared.Debugf("[devmapper] cancelDeferredRemoval END(%s)", info.Name())
...
func (devices *DeviceSet) Shutdown() error {
	shared.Debugf("[deviceset %s] Shutdown()", devices.devicePrefix)
	shared.Debugf("[devmapper] Shutting down DeviceSet: %s", devices.root)
	defer shared.Debugf("[deviceset %s] Shutdown() END", devices.devicePrefix)
	...
	for _, info := range devs {
		...
		if info.mountCount > 0 {
			...
			if err := syscall.Unmount(info.mountPath, syscall.MNT_DETACH); err != nil {
				shared.Debugf("Shutdown unmounting %s, error: %s", info.mountPath, err)
			}

			devices.Lock()
			if err := devices.deactivateDevice(info); err != nil {
				shared.Debugf("Shutdown deactivate %s , error: %s", info.Hash, err)
			}
			devices.Unlock()
		}
		info.lock.Unlock()
	...
	if info != nil {
		...
		devices.Lock()
		if err := devices.deactivateDevice(info); err != nil {
			shared.Debugf("Shutdown deactivate base , error: %s", err)
		}
		devices.Unlock()
		info.lock.Unlock()
	}
	...
	if devices.thinPoolDevice == "" {
		if err := devices.deactivatePool(); err != nil {
			shared.Debugf("Shutdown deactivate pool , error: %s", err)
		}
	}
...
func (devices *DeviceSet) MountDevice(hash, path, mountLabel string) error {
	shared.Debugf("MountDevice(%s, %s, %s)", hash, path, mountLabel)
	defer shared.Debugf("MountDevice(%s, %s, %s) DONE", hash, path, mountLabel)
	info, err := devices.lookupDevice(hash)
	if err != nil {
		return err
	}
	...
	options = joinMountOptions(options, mountLabel)

	shared.Debugf("about to Mount %s on %s with opts %s", info.DevName(), path, options)
	if err := syscall.Mount(info.DevName(), path, "ext4", flags, options); err != nil {
...
func (devices *DeviceSet) UnmountDevice(hash string) error {
	shared.Debugf("[devmapper] UnmountDevice(hash=%s)", hash)
	defer shared.Debugf("[devmapper] UnmountDevice(hash=%s) END", hash)
	...
	shared.Debugf("[devmapper] Unmount(%s)", info.mountPath)
	if err := syscall.Unmount(info.mountPath, syscall.MNT_DETACH); err != nil {
		return err
	}
	shared.Debugf("[devmapper] Unmount done")
...
func (devices *DeviceSet) getUnderlyingAvailableSpace(loopFile string) (uint64, error) {
	...
	if err := syscall.Statfs(loopFile, buf); err != nil {
		shared.Debugf("Couldn't stat loopfile filesystem %v: %v", loopFile, err)
		return 0, err
	}
...
func (devices *DeviceSet) isRealFile(loopFile string) (bool, error) {
	if loopFile != "" {
		...
		if err != nil {
			shared.Debugf("Couldn't stat loopfile %v: %v", loopFile, err)
			return false, err
		}
...
func (devices *DeviceSet) ExportDeviceName(hash string) (string, error) {
	info, err := devices.lookupDevice(hash)
	if err != nil {
		return "", err
	}
