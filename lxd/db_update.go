...
func dbUpdateFromV14(d *Daemon) error {
	...
	for _, cName := range cNames {
		...
		if !shared.PathExists(lvLinkPath) {
			shared.Log.Debug(" no lvpath . ignoring in lv upgrade", log.Ctx{"lvLinkPath": lvLinkPath, "cName": cName})
			continue
		}
